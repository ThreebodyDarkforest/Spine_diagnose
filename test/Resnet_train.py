...
def train(model: nn.Module, dataloader: DataLoader, eval_dataloader: DataLoader, \
          max_epochs=10, lr=1e-4, device='cpu', save=False, save_every=5, save_best=False):
    ...
    for epoch in range(max_epochs):
        ...
        val_acc = eval(epoch, loss, train_acc, model, eval_dataloader, device)
        if max_acc < val_acc and save_best:
            torch.save(model.state_dict(), './models/resnet50_best.pt')
        if (epoch + 1) % save_every == 0:
            torch.save(model.state_dict(), f'./models/resnet50_{epoch + 1}.pt')
...
def eval(epoch, loss, train_acc, model: nn.Module, dataloader: DataLoader, device='cpu'):
    ...
    with torch.no_grad():
        ...
        print('\t[epoch %d] train_loss: %.3f train_acc: %.3f val_acc: %.3f' %
          (epoch, loss, train_acc, val_acc))
    return val_acc

if __name__ == '__main__':
    model = torchvision.models.resnet50(pretrained=False) # torchvision.models.vit_b_32(pretrained=False)
    weight_path = os.path.join(PATH, 'models/resnet50.pth')
    ...
    model.to(device)
    train(model, dataloader, val_dataloader, 30, 1e-5, device=device, save=True, save_best=True)
